...
from utils.modeling import BertForSequenceClassificationEncoder, BertForSequenceClassificationEncoder_NL, FCClassifierForSequenceClassification
...
def init_glue_model_NL(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
    ...
    encoder_bert = BertForSequenceClassificationEncoder_NL(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers)
    classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    classifier_2 = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    classifier_3 = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    return encoder_bert, classifier, classifier_2, classifier_3
